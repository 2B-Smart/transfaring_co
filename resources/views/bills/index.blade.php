...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('bills.create') }}">إضافة سائق</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('bills.search') }}">
         ...
         @component('layouts.search', ['title' => 'Search'])
          @component('layouts.two-cols-search-row', ['items' => ['', ''],
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($bills as $bill)
                <tr role="row" class="odd">
                  ...
                  <td class="sorting_1">{{ $bill->driver_id }}</td>
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('bills.billlock',  $bill->id) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="LOCK">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        {{ if($bill->has_done==true){ }}
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('bills.destroy',  $bill->id) }}" onsubmit = "return confirm('Are you sure?')">
                        ...
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('bills.edit', $bill->id) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        تعديل
                        </a>
            ...
            <tfoot>
              <tr>
                ...
                <th class="hidden-xs" width="10%" rowspan="1" colspan="1">الرقم الوطني</th>
                <th class="hidden-xs" width="10%" rowspan="1" colspan="1">رقم الجوال</th>
                <th class="hidden-xs" width="10%" rowspan="1" colspan="1">رقم الجوال</th>
                <th class="hidden-xs" width="10%" rowspan="1" colspan="1">رقم الجوال</th>
